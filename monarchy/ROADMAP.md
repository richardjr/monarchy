...
Status 2026-09-10: steps 1 to 3 are running in the VM. Panel with focus tab, header, clock, workspace tabs, system meters and the stock widgets; window title bands as a per-screen Quickshell overlay (`shell/plugins/bridge/Bands.qml`) in the strip the Hyprland gaps reserve above every tiled window; the hybrid join (accent band meets the focus tab when the focused window abuts the panel, colour only otherwise); `[bridge]` theme keys for sizes and colours of tabs, bands and meters. Open: a font decision for Phosphor, the "another handler is registered for target omarchy.bar" warning (reproduced with the stock bar selected too, so it is upstream's), master count 2 (Hyprland has no config key for it; needs a keybinding or a startup `layoutmsg`), and a week of daily use.
