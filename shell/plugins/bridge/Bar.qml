...
// monarchy: the bridge console bar (monarchy.bridge). A fork of the stock
// omarchy.bar engine; the differences are marked with "monarchy:" comments so
// they can be re-applied when the stock engine changes. Everything else is
// upstream's code, kept verbatim so stock widgets keep working here.
Item {
  id: root

  // The omarchy-shell host injects omarchyPath from OMARCHY_PATH.
  // monarchy: plain properties, not `required`. The host loads a plugin bar
  // through a Loader by URL and assigns these in configureBar() after
  // creation, which QML refuses for required properties.
  ...
  // monarchy: panel width comes from the [bridge] section of shell.toml so
  // themes can tune it; the stock size-vertical stays for omarchy.bar.
  readonly property int bridgeWidth: {
    var v = Number(Color.shellValues["bridge.width"])
    return isFinite(v) && v > 0 ? Math.round(v) : 240
  }
  readonly property int barSize: vertical ? root.bridgeWidth : Style.bar.sizeHorizontal
  ...
  IpcHandler {
    // monarchy: own IPC target; the stock bar keeps omarchy.bar.
    target: "monarchy.bridge"
  ...
  component BarPanel: PanelWindow {
    ...
    WlrLayershell.namespace: "monarchy-bridge"
  ...
  component ModuleSlot: Item {
    ...
    readonly property var registryComponent: {
      // monarchy: the registry arrives after creation (see the properties above).
